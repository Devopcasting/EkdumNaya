...
class EAadhaarDocumentInfo:
    ...
    def _extract_e_aadhaar_dob(self) -> dict:
        ...
        try:
            ...
            dob_pattern = r'\b\d{2}/\d{2}/\d{4}|\b\d{2}/\d{5}|\b\d{2}-\d{2}-\d{4}|\b\d{2}/\d{2}/\d{2}|\b\d{1}/\d{2}/\d{4}|\b[Oo]?\d{1}/\d{5}\b'
    ...
    # Method to remove duplicate list of coordinates
    ...
    def collect_document_info(self) -> list:
        ...
        try:
            ...
            aadhaar_number = self._extract_e_aadhaar_number()
            if len(aadhaar_number['Coordinates']) == 0:
                self.logger.error("| Aadhaar Number not found in E-Aadhaar document")
                return {"message": "Aadhaar Number not found in E-Aadhaar document", "status": "REJECTED"}
            document_info_list.append(aadhaar_number)

            # Collect: Name
            aadhaar_name = self._extract_e_aadhaar_name()
            if len(aadhaar_name['Coordinates']) == 0:
                self.logger.error("| Name not found in E-Aadhaar document")
                return {"message": "Name not found in E-Aadhaar document", "status": "REJECTED"}
            document_info_list.append(aadhaar_name)

            # Collect: Date of Birth
            aadhaar_dob = self._extract_e_aadhaar_dob()
            if len(aadhaar_dob['Coordinates']) == 0:
                self.logger.error("| Date of Birth not found in E-Aadhaar document")
                return {"message": "Date of Birth not found in E-Aadhaar document", "status": "REJECTED"}
            document_info_list.append(aadhaar_dob)

            # Collect: Gender
            aadhaar_gender = self._extract_e_aadhaar_gender()
            if len(aadhaar_gender['Coordinates']) == 0:
                self.logger.error("| Gender not found in E-Aadhaar document")
                return {"message": "Gender not found in E-Aadhaar document", "status": "REJECTED"}
            document_info_list.append(aadhaar_gender)

            # Collect: Address
            aadhaar_address = self._extract_e_aadhaar_address()
            ...
            # Collect: Pincode
            aadhaar_pincode = self._extract_e_aadhaar_pincode()
            ...
            # Collect: Mobile
            aadhaar_mobile = self._extract_e_aadhaar_mobile()
            if len(aadhaar_mobile['Coordinates']) == 0:
                self.logger.error("| Mobile not found in E-Aadhaar document")
            else:
                document_info_list.append(aadhaar_mobile)
            
            # Collect: QRCodes
            aadhaar_qrcodes = self._extract_e_aadhaar_qr_codes()
            ...
            return {"message": "Successfully Processed E-Aadhaar Documment", "status": "REDACTED", "data": unique_document_info_list}
        except Exception as e:
